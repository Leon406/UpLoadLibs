...
//                .theme(R.style.picture_QQ_style)
    ...
    public static Single<List<String>> openDynamicGallery(Activity activity) {
        if (!checkPermission(activity)) {
            return null;
        }
...
//                .selectionMedia(selectedList)
    ...
    public static Maybe<List<Uri>> openGallery(Activity activity) {
        if (!checkPermission(activity)) {
            return null;
        }
        return PictureSelector.create(activity)
...
//                .selectionMedia(selectedList)
                ...
                .flatMap(result -> Observable.fromIterable(
                        PictureSelector.obtainMultipleResult(result.data()))
                        // .map(image -> image.getPath())
                ).map(media -> Uri.fromFile(new File(media.getPath())))
                .toList()
